...
gerar_bootstrap <- function(df, tamanhoAmostra) {
    bootstrap <- sample_n(df, size = tamanhoAmostra, replace = TRUE);
    bootstrap <- bootstrap %>% mutate(prop_news = news / (old + news))
    return(bootstrap)
}

calcula_theta <- function(col) {
    return(mean(col))
}

reamostragens = tibble(i = 1:repeticoes) %>% 
  mutate(theta_c_s = map_dbl(i, ~ calcula_theta(gerar_bootstrap(lastfm, tamanhoAmostra) %>% pull(prop_news))))
